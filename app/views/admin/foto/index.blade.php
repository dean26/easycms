    ...
    <div class="col_12">
        <ol class="breadcrumb">
            <li><a href="<?php echo AppHelper::UrlTo('/foto?ref_id=' . $data['ref_id'] . '&ref=' . $data['ref'])?>">Zdjęcia
                    dla obiektu {{ $ref_obj }}</a></li>
            <li class="active">Lista</li>
        </ol>
        <h1>Zdjęcia dla obiektu {{ $ref_obj }} <a
                ...
                @if(count($lista["wyniki"]) > 0)
                    ...
                    <ol id="lista_f" class="example">
                        @foreach($lista["wyniki"] as $rec)
                            <li id="foto_{{ $rec->id }}">
                                @if($rec->plik)
                                    <a data-title="{{ $rec->opis }}" href="<?php echo AppHelper::BaseUrl(); ?>public/uploads/galeria/<?php echo $rec->plik ?>" data-lightbox="galeria">
