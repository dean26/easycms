...
class Validation
{
        ...
        foreach ($this->errors as $e) {
            ...
            switch ($e['rule']) {
                ...
                case 'validate_required' :
                    $resp[$e['field']] = "Pole jest wymagane.";
                    break;
                case 'validate_compare' :
                    $resp[$e['field']] = "Pole musi zawierać tą samą wartość co $param.";
                    break;
                case 'validate_uniq' :
                    $resp[$e['field']] = "Wybrana wartość już istnieje.";
                    break;
                case 'validate_valid_email':
                    $resp[$e['field']] = "Pole musi być poprawnym adresem email.";
                    break;
                case 'validate_max_len':
                    $resp[$e['field']] = "Pole może zawierać maksymalnie $param znaków.";
                    break;
                case 'validate_min_len':
                    $resp[$e['field']] = "Pole musi zawierać minimum $param znaków.";
                    break;
                case 'validate_exact_len':
                    $resp[$e['field']] = "Pole needs to be exactly $param characters in length";
                    break;
                case 'validate_alpha':
                    $resp[$e['field']] = "Pole may only contain alpha characters(a-z)";
                    break;
                case 'validate_alpha_numeric':
                    $resp[$e['field']] = "Pole nie może zawierać znaków specjalnych ani polskich liter.";
                    break;
                case 'validate_alpha_dash':
                    $resp[$e['field']] = "Pole może zawierać tylko znaki [a-zA-z0-9_-]";
                    break;
                case 'validate_numeric':
                    $resp[$e['field']] = "Pole may only contain numeric characters";
                    break;
                case 'validate_integer':
                    $resp[$e['field']] = "Pole may only contain a numeric value";
                    break;
                case 'validate_boolean':
                    $resp[$e['field']] = "Pole may only contain a true or false value";
                    break;
                case 'validate_float':
                    $resp[$e['field']] = "Pole may only contain a float value";
                    break;
                case 'validate_valid_url':
                    $resp[$e['field']] = "Pole is required to be a valid URL";
                    break;
                case 'validate_url_exists':
                    $resp[$e['field']] = "The <span class=\"$field_class\">$field</span> URL does not exist";
                    break;
                case 'validate_valid_ip':
                    $resp[$e['field']] = "Pole needs to contain a valid IP address";
                    break;
                case 'validate_valid_cc':
                    $resp[$e['field']] = "Pole needs to contain a valid credit card number";
                    break;
                case 'validate_valid_name':
                    $resp[$e['field']] = "Pole needs to contain a valid human name";
                    break;
                case 'validate_contains':
                    $resp[$e['field']] = "Pole needs to contain one of these values: ".implode(', ', $param);
                    break;
                case 'validate_contains_list':
                    $resp[$e['field']] = "Pole needs to contain a value from its drop down list";
                    break;
                case 'validate_doesnt_contain_list':
                    $resp[$e['field']] = "Pole contains a value that is not accepted";
                    break;
                case 'validate_street_address':
                    $resp[$e['field']] = "Pole needs to be a valid street address";
                    break;
                case 'validate_date':
                    $resp[$e['field']] = "Pole needs to be a valid date";
                    break;
                case 'validate_min_numeric':
                    $resp[$e['field']] = "Pole needs to be a numeric value, equal to, or higher than $param";
                    break;
                case 'validate_max_numeric':
                    $resp[$e['field']] = "Pole needs to be a numeric value, equal to, or lower than $param";
                    break;
                case 'validate_starts':
                    $resp[$e['field']] = "Pole needs to start with $param";
                    break;
                case 'validate_extension':
                    $resp[$e['field']] = "Dozwolone formaty plików to: $param";
                    break;
                case 'validate_required_file':
                    $resp[$e['field']] = "Pole jest wymagane.";
                    break;
                case 'validate_equalsfield':
                    $resp[$e['field']] = "Pole does not equal $param field";
                    break;
                case 'validate_min_age':
                    $resp[$e['field']] = "Pole needs to have an age greater than or equal to $param";
                    break;
                case 'validate_filesize':
                    $resp[$e['field']] = "Maksymalna wielkość tego pliku wynosi $param";
                    break;
                default:
                    $resp[$e['field']] = "Pole is invalid";
            }
        }
    ...
    */
    ...
    protected function validate_uniq($field, $input, $param = null)
    {
        if (!isset($input[$field])) {
            return;
        }

        if ($param == 0) { // valid, return nothing
            return;
        }
    ...
    protected function validate_compare($field, $input, $param = null)
    {
        if (!isset($input[$field])) {
            return;
        }
        ...
        if ($input[$field] == $input[$param]) { // valid, return nothing
            return;
        }
    ...
    protected function validate_max_len($field, $input, $param = null)
    {
        ...
        if(mb_strlen($input[$field]) == 0) return;
    ...
    protected function validate_filesize($field, $input, $param = null)
    {
        ...
        if ($file->getError() !== 4) {
            ...
            $param_size = 0;
            if($dane[1] == "MB"){
                $param_size = $dane[0] * 1048576;
            } elseif($dane[1] == "KB"){
                $param_size = $dane[0] * 1024;
            } else {
                $param_size = $dane[0];
            }

            if($file->getSize() <= $param_size){
                return;
            }
